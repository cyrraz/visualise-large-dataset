...
for batch in parquet_file.iter_batches(batch_size=batch_size):
    ...
    hist_normal.fill(variable[category==0], weight=weight[category==0])
    hist_exponential.fill(variable[category==1], weight=weight[category==1])
    hist_log_normal.fill(variable[category==2], weight=weight[category==2])

fig, ax = plt.subplots()

plot_hist([hist_normal, hist_exponential, hist_log_normal], ax=ax, stacked=True, label=["Gaussian", "Exponential", "Log-normal"])
